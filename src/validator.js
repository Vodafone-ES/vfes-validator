...
export default class Validator {
  constructor(form, options) {
    ...
    this.onFocusout = options.onfocusout && typeof options.onfocusout === 'boolean' ? options.onfocusout : null;
  ...
  validate(input) {
    const mod = this;
    const ruleNames = Object.keys(mod.rules);
   ...
   *
   * @description Asociamos el evento focusout al input recibido.
   *
   * @param { HTMLElement } input - input del formulario
   *
   * @returns { undefined }
   */
  onFocusoutHandler(input) {
    input.addEventListener('focusout', this.validate(input));
  }
  ...
  init() {
    ...
    /* if(mod.onfocusout) {
			names.forEach(name => {
				let input = mod.getInput(name);

				if(input) {
					mod.onFocusoutHandler(input);
				}
			});
		} */
  }
